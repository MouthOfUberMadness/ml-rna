...
class dNdS_from_MutSel():
    ...
    def __init__(self, codon_frequencies, mutation_dictionary=None):
        ...
        self.ZERO = 1e-10
        self.codons = ["AAA", "AAC", "AAG", "AAT", "ACA", "ACC", "ACG", "ACT", "AGA", "AGC", "AGG", "AGT", "ATA", "ATC", "ATG", "ATT", "CAA", "CAC", "CAG", "CAT", "CCA", "CCC", "CCG", "CCT", "CGA", "CGC", "CGG", "CGT", "CTA", "CTC",
        ...
        self.codon_dict = {"AAA": "K", "AAC": "N", "AAG": "K", "AAT": "N", "ACA": "T", "ACC": "T", "ACG": "T", "ACT": "T", "AGA": "R", "AGC": "S", "AGG": "R", "AGT": "S", "ATA": "I", "ATC": "I", "ATG": "M", "ATT": "I", "CAA": "Q", "CAC": "H", "CAG": "Q", "CAT": "H", "CCA": "P", "CCC": "P", "CCG": "P", "CCT": "P", "CGA": "R", "CGC": "R", "CGG": "R", "CGT": "R", "CTA": "L", "CTC": "L",
        ...
        if type(codon_frequencies) is dict:
            ...
            # print(self.codon_freqs_dict.values())
        # assert(abs(1. - np.sum(self.codon_freqs_dict.values())) <
        #        self.ZERO), "\n\nProvided codon frequencies must sum to 1."

        # Mutation setup
        if mutation_dictionary is None:
            self.mu_dict = {'AC': 1.,  'CA': 1.,  'AG': 1.,  'GA': 1.,  'AT': 1.,
                            'TA': 1.,  'CG': 1.,  'GC': 1.,  'CT': 1.,  'TC': 1.,  'GT': 1.,  'TG': 1.}
        else:
    ...
    def calc_paths(self, source, type):
        ...
        for target in self.codons:
            # only consider single nucleotide differences since are calculating instantaneous.
            diff = self.get_nuc_diff(source, target)
            if (type == 'nonsyn' and self.codon_dict[source] != self.codon_dict[target]) or (type == 'syn' and self.codon_dict[source] == self.codon_dict[target]):
                if len(diff) == 2:
                    rate += self.calc_subst_prob(
                        source_freq, self.codon_freqs_dict[target], self.mu_dict[diff], self.mu_dict[diff[1]+diff[0]])
